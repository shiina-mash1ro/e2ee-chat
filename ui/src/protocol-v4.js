...
const EXACT_BINARY_FIELD_LENGTHS = {
  public_key: 32,
  sign_public_key: 32,
  hello_mac: 32,
  signature: 64,
...
export function createProtocolV4(sodium) {
  const decodeWireBytes = (value, field = "") => {
    ...
    const exact = EXACT_BINARY_FIELD_LENGTHS[field];
    if (exact && bytes.byteLength !== exact) throw new RangeError(`Invalid ${field} length`);
    return bytes;
  };
  ...
  const normalizeWireEvent = (event) => {
    ...
    const normalized = { ...event, protocol: PROTOCOL_VERSION };
